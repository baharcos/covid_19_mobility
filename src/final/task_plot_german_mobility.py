...
# Function for plots
titles = ['Retail and Recreation (7d-average)', 'Grocery and Pharmacy (7d-average)', 'Workplaces (7d-average)', 'Parks (7d-average)', 'Residential (7d-average)', 'Transit Stations (7d-average)']
...
def mobility_plot(data_set, var_list_moving_avg, path, fig_width = 20, fig_height = 40,group_var="state"):
    ...
    for i in range(num_plots):
        sns.lineplot(x=data_set.index, y=var_list_moving_avg[i],data=data_set, hue=group_var,ax=ax[i])
...
def task_plot_german_mobility(depends_on, produces):
    ...
    sns.lineplot(data=germany_country_level_data[['retail_and_recreation_avg_7d',
        'grocery_and_pharmacy_avg_7d', 'parks_avg_7d',
        'transit_stations_avg_7d', 'workplaces_avg_7d', 'residential_avg_7d']])

    ax.axhline(0,color="black",alpha=0.8)
    ax.axvspan(pd.to_datetime('2020-03-22'),pd.to_datetime('2020-06-15'),alpha=0.2,color="lightgrey")
    ax.axvspan(pd.to_datetime('2020-11-02'),pd.to_datetime('2021-03-01'),alpha=0.2,color="lightgrey")
    ax.axvspan(pd.to_datetime('2020-12-16'),pd.to_datetime('2021-03-01'),alpha=0.4,color="lightgrey")
...
products = {
    ...
    "city_vs_territorial_state": BLD/"figures"/"German_Mobility"/"city_vs_territorial_state.png",
