...
def mobility_plot(
    ...
    """Creating multiple plots in one figure for a given data frame, titles and
    colors have to be defined
...
#     mobility_plot(
#         data_set=germany_state_level.groupby(["date", "brd_ddr"]).mean().
# reset_index(1),
#         var_list_moving_avg=varlist_moving_avg,
#         titles=titles,
#         colors=colors,
...
def task_plot_european_countries(depends_on, produces):
    ...
    eu_complete_data = eu_complete_data.loc[
        eu_complete_data["state"] == "country",
    ]
